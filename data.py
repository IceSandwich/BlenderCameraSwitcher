...
class ICECS_DT_data(bpy.types.PropertyGroup):
    ...
    @classmethod
    def updateData(cls, context:bpy.types.Context):
        data = cls.getOrNewProperty(context.scene)
        data.ResX = context.scene.render.resolution_x
        data.ResY = context.scene.render.resolution_y
        data.CameraName = context.scene.camera.name if context.scene.camera else ""
        print("update to %d, %d, %s"%(data.ResX, data.ResY, data.CameraName))

    @classmethod
    def recoverData(cls, context:bpy.types.Context):
        data = context.scene.IceCS_data
        context.scene.render.resolution_x = data.ResX
        context.scene.render.resolution_y = data.ResY
        context.scene.camera = context.blend_data.objects.get(data.CameraName)
        print("recover to %d, %d, %s"%(data.ResX, data.ResY, data.CameraName))
